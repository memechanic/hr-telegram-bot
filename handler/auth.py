...
async def start_pincode(message: Message, command: CommandObject):
    ...
    if success:
        ...
        await message.answer("Пинкод использован либо его не существует")

# авторизация посредством ввода пинкода пользователем
...
async def create_invite_link(message: Message, bot: Bot):
    ...
    if link:
        text = f"Сгенерированный пинкод: <code>{pincode}</code>\nCсылка для сотрудника:\n<a>{link}</a>"
